...
void process_videos(
            ...
            const int32_t left_video_offset,
            const int32_t right_video_offset,
    ...
    ThreadSafeQueue<PanoramicPacket> pano_queue(2);
    for(uint16_t i=0; i < nb_stitch_workers; ++i) {
        unique_ptr<FrameStitcher> fs(new FrameStitcher(false, pano_offset_x, pano_offset_y, pano_width, pano_height, stitcher_queue, pano_queue, cameras_params, camera_intrinsics_K, camera_intrinsics_distortion_coefficients, camera_intrinsics_image_size_used, masks_warped, reference_bgr_value_idxs, reference_bgr_cumsum));
        fs->start();
    ...
    while(true) {
        ...
        while(read_audio) {
            read_audio = false;
            unique_ptr<AVPacket, PacketDeleter> left_audio_packet(left_processor.getOutAudioQueue().pop(chrono::milliseconds(1)));
            if(left_audio_packet) {
                read_audio = true;
            }

            unique_ptr<AVPacket, PacketDeleter> right_audio_packet(right_processor.getOutAudioQueue().pop(chrono::milliseconds(1)));
            if(right_audio_packet) {
                read_audio = true;
            }
        }
        ...
        while(read_frame) {
            read_frame = false;
            unique_ptr<PanoramicPacket> panoramic_packet(pano_queue.pop(chrono::milliseconds(1)));
            if(panoramic_packet) {
                read_frame = true;
            }
        }
...
int main(int argc, const char ** argv) {
    ...
    process_videos(left_filename, right_filename, left_video_offset, right_video_offset, camera_params_filename, camera_intrinsics_filename, nb_stitch_workers, nb_encoding_threads, fix_exposure);
    spdlog::info("Done");
}
